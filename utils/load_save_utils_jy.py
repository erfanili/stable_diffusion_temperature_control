...
from models.sd1_5.storage_sd1_5 import AttnFetchSDX_2, AttnFetchSDX
...
def save_maps_new(all_maps,
              idx,
              gif_save_dir = './gifs',
              file_name = f'{0}',
              save_maps_by_time = False,
              num_inference_steps=50):
    ## conform attn map format
    idx_maps = maps_to_images_new(attn_array = all_maps,idx = idx)
    os.makedirs(gif_save_dir,exist_ok=True)
    save_gif(images = idx_maps, directory=gif_save_dir, file_name = file_name)
    if save_maps_by_time:
        for t in range(num_inference_steps):
...
def maps_to_images_new(attn_array,idx):
    '''conform attn map'''
    map_list = []
    for t in range(len(attn_array[0].keys())):
        map_t = attn_array[0][t][:,:,idx].reshape(-1).numpy()
