...
# def evaluate_prompt(prompt,model,preprocess,device,config):
...
methods = ['Conform', 'Linguistic_Binding', 'ours_SA_guide']
categories = ['animals', 'objects']

for method in methods:
    for category in categories:
        path = os.path.join(image_dir,method,category)
        images = get_file_names(path)

# for prompt in tqdm(prompt_list):

#     results_per_prompt[prompt] = evaluate_prompt(prompt=prompt, device=device, model=model, preprocess=preprocess)
