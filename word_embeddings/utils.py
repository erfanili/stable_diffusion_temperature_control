...
def load_model(model_to_load,device):
    model_id = "openai/clip-vit-large-patch14"
    

    if model_to_load == 'clip_text_model':
        model = CLIPTextModel.from_pretrained(pretrained_model_name_or_path=model_id,device_map = device)
    elif model_to_load == 'tokenizer':
        model = CLIPTokenizer.from_pretrained(pretrained_model_name_or_path=model_id, device_map = device)
    elif model_to_load == 'sd_pipeline':
        model_id = "runwayml/stable-diffusion-v1-5"
        model = StableDiffusionPipeline.from_pretrained(pretrained_model_name_or_path=model_id)
        model.to(device)
    elif model_to_load == 'T5_text_encoder':
    ...
    elif model_to_load == "pixart_pipeline":
        model_id = "PixArt-alpha/PixArt-XL-2-1024-MS"
        model = PixArtAlphaPipeline.from_pretrained(model_id, device_map = 'auto')
        # model.to(device)
    return model
...
def save_image(image,dir,file_name):
    ...
    file_path = os.path.join(dir,format_time())
    image.save(f'{file_path}_{file_name}.jpg')
